   ...
   int pX = 0; int pY = 0;
    ...
    private void rotateGimbalToMin (Button button) {
        ...
        if (key == DJIGimbalCapabilityKey.AdjustPitch){
            ...
            mPitchRotation.angle = -45; //TODO minValue.floatValue()
        }

        if(key == DJIGimbalCapabilityKey.AdjustYaw){
            mYawRotation.direction = DJIGimbalRotateDirection.Clockwise;
            mYawRotation.angle = 90; //TODO minValue.floatValue()
        }
...
//        }
    ...
    private void rotateGimbalToMax(Button button) {
        ...
        if (key == DJIGimbalCapabilityKey.AdjustPitch){
            mPitchRotation.direction = DJIGimbalRotateDirection.Clockwise;
            mPitchRotation.angle = maxValue.floatValue(); //TODO maxValue.floatValue()
        }

        if(key == DJIGimbalCapabilityKey.AdjustYaw){
            mYawRotation.direction = DJIGimbalRotateDirection.Clockwise;
            mYawRotation.angle = -90; //TODO maxValue.floatValue()
        }
...
//        }
    ...
    public void onClick(View v, MotionEvent event){
        int button = event.getAction();
        switch (v.getId()) {
            case R.id.btn_pitchMin:{
                if(button == MotionEvent.ACTION_DOWN) {
                    pY = -2;
                } else if(button == MotionEvent.ACTION_UP){
                    pY = 0;
                }
